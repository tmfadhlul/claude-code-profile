...
import { Boxes, LoaderCircle, Plus, RefreshCw, Server } from 'lucide-react'
...
export function McpPage() {
  ...
  const [f, setF] = useState({ name: '', command: 'npx', args: '' }); const [from, setFrom] = useState('')
  ...
  useEffect(() => { load() }, [])
  const toggle = async (server: string, profile: string, on: boolean) => { const key = `${server}:${profile}`; setCellBusy(key, true); try { on ? await api.addMcp({ name: server, targets: [profile] }) : await api.rmMcp(server, [profile]); await load() } catch (e: any) { toast.error(e.message) } finally { setCellBusy(key, false) } }
  const runSync = async () => { setSyncing(true); try { await api.syncMcp(from, 'all'); toast.success(`Synced from ${from}`); setConfirmSync(false); await load() } catch (e: any) { toast.error(e.message) } finally { setSyncing(false) } }
  ...
  return <>
    ...
    {!data ? <LoadingState label="Loading MCP assignments" /> : data.servers.length === 0 ? <EmptyState icon={Boxes} title="No MCP servers" description="Add a server once, then choose which profiles can use it." action={<Button onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> Add server</Button>} /> : <div className="space-y-5">
      ...
      <div className="hidden overflow-x-auto rounded-xl border bg-card md:block"><table className="w-full border-collapse text-sm"><thead><tr className="border-b bg-muted/35"><th className="sticky left-0 z-10 min-w-48 bg-muted px-5 py-3 text-left text-[0.65rem] font-bold uppercase tracking-wider text-muted-foreground">Server</th>{data.profiles.map(p => <th key={p.name} className="min-w-32 px-4 py-3 text-center text-[0.65rem] font-bold uppercase tracking-wider text-muted-foreground">{p.name}</th>)}</tr></thead><tbody>{data.servers.map(s => <tr key={s} className="border-b last:border-0 hover:bg-muted/20"><td className="sticky left-0 bg-card px-5 py-4"><span className="flex items-center gap-2 font-mono text-xs font-bold"><Server className="h-3.5 w-3.5 text-primary" />{s}</span></td>{data.profiles.map(p => { const key = `${s}:${p.name}`; const pending = busy.has(key); return <td key={p.name} className="px-4 py-4 text-center"><span className="relative inline-flex items-center justify-center">{pending && <LoaderCircle className="absolute h-3.5 w-3.5 animate-spin text-primary" aria-hidden />}<Switch checked={p.has.includes(s)} disabled={pending} onCheckedChange={on => toggle(s, p.name, on)} aria-label={`${p.has.includes(s) ? 'Disable' : 'Enable'} ${s} for ${p.name}`} className={pending ? 'opacity-0' : undefined} /></span></td> })}</tr>)}</tbody></table></div>

      <div className="space-y-3 md:hidden">{data.servers.map(s => <section key={s} className="surface-flat overflow-hidden"><div className="flex items-center gap-2 border-b bg-muted/30 px-4 py-3 font-mono text-sm font-bold"><Server className="h-4 w-4 text-primary" />{s}</div><div className="divide-y">{data.profiles.map(p => { const key = `${s}:${p.name}`; const pending = busy.has(key); return <label key={p.name} className="flex items-center justify-between px-4 py-3 text-sm font-semibold"><span>{p.name}</span><span className="relative inline-flex items-center justify-center">{pending && <LoaderCircle className="absolute h-3.5 w-3.5 animate-spin text-primary" aria-hidden />}<Switch checked={p.has.includes(s)} disabled={pending} onCheckedChange={on => toggle(s, p.name, on)} className={pending ? 'opacity-0' : undefined} /></span></label> })}</div></section>)}</div>
    </div>}

    <ConfirmDialog open={confirmSync} onOpenChange={o => { if (!syncing) setConfirmSync(o) }} title={`Sync all profiles from "${from}"?`} description="Replaces every other profile's MCP assignments with this profile's toolset. This cannot be undone from here." confirmLabel="Sync to all" destructive busy={syncing} onConfirm={runSync} />
  </>
}
