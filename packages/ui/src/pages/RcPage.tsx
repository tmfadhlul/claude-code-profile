...
function Block({ title, note, lines, otherLines, tint }: { title: string; note: string; lines: string[]; otherLines: Set<string>; tint: string }) {
  return <section className="min-w-0 flex-1 overflow-hidden rounded-xl border bg-[#242720] text-[#f5f0e5]"><div className="border-b border-white/10 px-4 py-3"><h2 className="text-sm font-bold">{title}</h2><p className="mt-0.5 text-[0.68rem] text-white/40">{note}</p></div><pre className="max-h-[520px] overflow-auto p-4 text-xs leading-6"><code>{lines.map((l, i) => <span key={i} className={cn('block min-w-max rounded px-1', !otherLines.has(l) && l.trim() !== '' && tint)}>{l || ' '}{'\n'}</span>)}</code></pre></section>
}
