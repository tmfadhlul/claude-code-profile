...
import { ConfirmDialog, StatusPill } from '@/components/Page'
...
export function ProfileEditor({ profile, profiles, servers, secretNames, onClose, onSaved }: { profile: ProfileRow; profiles: ProfileRow[]; servers: string[]; secretNames: string[]; onClose: () => void; onSaved: () => void }) {
  ...
  // Batched PATCH (Task 11): send the full desired profile — including the whole mcp set — as one
  // call instead of a patch followed by an addMcp/rmMcp loop.
  const save = async () => { for (const r of [...env, ...padv]) if (r.secret && !r.value) { toast.error(`Pick a secret for ${r.key || 'environment variable'}`); return } setSaving(true); try { const linksObj: Record<string, string> = {}; for (const r of links) if (r.key.trim()) linksObj[r.key.trim()] = r.value; await api.patchProfile(profile.name, { env: fromEnvRows(env), settingsEnv: profile.agent === 'claude' ? mergeProviderEnv(pform, fromEnvRows(padv)) : undefined, links: linksObj, launcher: launcher.trim() || null, skipPermissions: launcher.trim() ? skipPermissions : false, sharedSessions, mcp }); toast.success(`Saved ${profile.name}`); onSaved() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) } }
  ...
  <ConfirmDialog open={confirmSkip} onOpenChange={setConfirmSkip} title="Skip permission prompts?" description="This bypasses every confirmation for this profile's launcher. Enable only for fully trusted profiles." confirmLabel="Enable" destructive onConfirm={() => { setSkipPermissions(true); setConfirmSkip(false) }} />
  </>
}
